...
export default function ServicesPage() {
  ...
  const deleteSpecialist = async (id: string) => {
      if (!confirm("Are you sure you want to REJECT and DELETE this service? This cannot be undone.")) return;
  ...
  const filteredSpecialists = specialists.filter((specialist) => {
    // 1. Filter by Tab
    if (activeTab === "All") {
        // "All" view for Admin should NOT show Drafts. 
        // Logic: Show everything that has been PUBLISHED (Submitted) by the user.
        if (specialist.is_draft) return false;
    }
    
    if (activeTab === "Published") {
        if (specialist.is_draft) return false;
        if (specialist.verification_status !== "VERIFIED") return false;
    }
  ...
  return (
    <div className="flex-1 bg-white min-h-screen font-sans text-gray-900 px-6 py-8">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Specialists Services</h1>
        <p className="mt-1 text-sm text-gray-500">
          Create and manage your service offerings for Clients & Companies
        </p>
      </div>
      ...
      <div className="overflow-x-auto min-h-[400px]">
        {loading ? (
           <div className="flex items-center justify-center h-full text-gray-500 text-sm">Loading services...</div>
        ) : filteredSpecialists.length === 0 ? (
           <div className="flex items-center justify-center h-full text-gray-500 text-sm py-10">No Specialist Services found.</div>
        ) : (
                ...
                <td className="py-4 px-3 text-center">
                  <span
                    className={`
                      ...
                      ${!specialist.is_draft ? "bg-blue-500" : "bg-gray-400"}
                    `}
                  >
                    {specialist.is_draft ? "Draft" : (specialist.verification_status === "VERIFIED" ? "Published" : "Submitted")}
                  </span>
                </td>
                <td className="py-4 px-3 text-center relative">
                  ...
                  {openActionId === specialist.id && (
                    <div className="absolute right-8 top-1/2 -translate-y-1/2 w-40 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden">
                      ...
                      <button 
                        onClick={() => router.push(`/admin/service?id=${specialist.id}`)}
